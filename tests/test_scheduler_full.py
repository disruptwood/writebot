...
class TestSendEveningWarning:
    async def test_no_missing_members(self):
        ...
        await queries.upsert_daily_participation(100, "2024-01-15", 50)
        await queries.update_streak(100, "alice", "Alice", 1, 1, "2024-01-15")

        bot = make_bot()
        await _send_evening_warning(bot, "2024-01-15")

        bot.send_message.assert_not_called()
        assert await queries.get_state(STATE_LAST_WARNING_DATE) == "2024-01-15"

    async def test_missing_members_warned(self):
        """Members who haven't written today should get a warning."""
        await queries.activate_member(100, "alice", "Alice", source="test",
                                      joined_at=datetime(2024, 1, 1, tzinfo=ZoneInfo("UTC")))
        await queries.activate_member(200, "bob", "Bob", source="test",
                                      joined_at=datetime(2024, 1, 1, tzinfo=ZoneInfo("UTC")))
        await queries.upsert_daily_participation(100, "2024-01-15", 50)
        await queries.update_streak(100, "alice", "Alice", 1, 1, "2024-01-15")
        ...
        # Bob has username so shows as @bob
    ...
    async def test_kick_warning_included_for_overdue_members(self):
        ...
        # Alice last posted on Jan 12, evaluation is Jan 15 — overdue
        await queries.update_streak(100, "alice", "Alice", 0, 1, "2024-01-12")
...
class TestKickMember:
    async def test_successful_kick(self):
        ...
        bot = make_bot()
        result = await _kick_member(bot, {"user_id": 100})
...
class TestRunMidnightEnforcement:
    async def test_kicks_overdue_members(self):
        ...
        await queries.activate_member(100, "alice", "Alice", source="test",
                                      joined_at=datetime(2024, 1, 1, tzinfo=ZoneInfo("UTC")))
        # No posts at all — overdue
        await queries.set_state("main_join_request_invite_link", "https://t.me/+test")

        bot = make_bot()
        await _run_midnight_enforcement(bot, "2024-01-15")

        member = await queries.get_member(100)
        assert member["status"] == "kicked"
        bot.send_message.assert_called_once()
        assert await queries.get_state(STATE_LAST_KICK_DATE) == "2024-01-15"
