...
async def _needs_multi_channel_migration(db: aiosqlite.Connection) -> bool:
    """Check if the DB has old single-channel schema that needs migration."""
    if not await _table_exists(db, "members"):
        return False  # Fresh DB, will be created with new schema

    columns = await _get_columns(db, "members")
    if "channel_id" not in columns:
        return True  # Old schema without channel_id

    # Check if PK includes channel_id
    async with db.execute("PRAGMA table_info(members)") as cursor:
        rows = await cursor.fetchall()
    pk_cols = [row[1] for row in rows if row[5] > 0]
    return "channel_id" not in pk_cols
...
async def _do_multi_channel_migration(db: aiosqlite.Connection):
    ...
    # --- channel_posts ---
    if await _table_exists(db, "channel_posts"):
    ...
    # --- daily_participation ---
    if await _table_exists(db, "daily_participation"):
    ...
    # --- streaks ---
    if await _table_exists(db, "streaks"):
    ...
    if await _table_exists(db, "members"):
        ...
        await _ensure_column(db, "members", "last_status_changed_at", "TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
    ...
    # --- member_events ---
    if await _table_exists(db, "member_events"):
    ...
    if await _table_exists(db, "bot_state"):
...
async def init_db():
    ...
    async with aiosqlite.connect(db_path) as db:
        # Run multi-channel migration FIRST if needed (before creating new-schema tables)
        if await _needs_multi_channel_migration(db):
            await _do_multi_channel_migration(db)

        # Create tables with new schema (only creates if they don't exist)
        await db.execute("""
        ...
        # Create indexes AFTER tables are guaranteed to have the right schema
        await db.execute("""
